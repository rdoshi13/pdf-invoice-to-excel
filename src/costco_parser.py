...
DATE_PATTERNS = [
    re.compile(r"\bOrder\s+date\s*:?\s*(\d{1,2}/\d{1,2}/\d{4})", re.IGNORECASE),
    re.compile(r"\bOrder\s+date\s*:?\s*([A-Z][a-z]+\s+\d{1,2},\s+\d{4})", re.IGNORECASE),
    re.compile(r"\bDate\s*:?\s*(\d{1,2}/\d{1,2}/\d{4})", re.IGNORECASE),
]
...
def parse_invoice_text(raw_text: str, source_path: Path) -> Invoice:
    if "costco" not in raw_text.lower():
        raise ParseError("Not a Costco invoice")

    order_date = _parse_order_date(raw_text)
    items = _parse_items(raw_text)
    if not items:
        raise ParseError("No Costco item rows found")
...
def _parse_order_date(raw_text: str):
    for pattern in DATE_PATTERNS:
        ...
        return parse_date_text(date_text, ("%m/%d/%Y", "%B %d, %Y", "%b %d, %Y"))

    raise ParseError("Costco order date not found")
...
def _parse_items(raw_text: str) -> list[InvoiceItem]:
    ...
    for raw_line in raw_text.splitlines():
        ...
        match = ITEM_QTY_RE.match(line) or ITEM_TOTAL_RE.match(line)
        if not match:
            continue
